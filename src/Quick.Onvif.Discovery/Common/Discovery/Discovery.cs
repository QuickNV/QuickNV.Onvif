...
public class Discovery : IDisposable
{
    ...
    public const string SCOPE_MATCH_BY_URL = "http://schemas.xmlsoap.org/ws/2005/04/discovery/rfc3986";

    public readonly IPAddress WS_DISCOVER_MULTICAST_IP4 = IPAddress.Parse("239.255.255.250");

    public readonly IPAddress WS_DISCOVER_MULTICAST_IP6 = IPAddress.Parse("FF02::C");
    ...
    private void SendToLocal3702(byte[] msg)
    {
        ...
        foreach (NetworkInterface networkInterface in allNetworkInterfaces)
        {
            ...
            foreach (UnicastIPAddressInformation unicastAddress in networkInterface.GetIPProperties().UnicastAddresses)
            {
                ...
                {
                    socket.SendTo(msg, new IPEndPoint(unicastAddress.Address, WS_DISCOVER_PORT));
                }
            }
        }
    }
    ...
    public void Probe(bool multicast, IPAddress address, string deviceId, int timeout, DiscoveryUtils.DiscoveryType[][] types, string[] scopes, string matchRule)
    {
        ...
        try
        {
            ...
            IPEndPoint destination = (multicast ? new IPEndPoint(GetDiscoveryMulticastAddress(), WS_DISCOVER_PORT) : new IPEndPoint(address, WS_DISCOVER_PORT));
